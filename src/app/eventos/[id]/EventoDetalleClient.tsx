...
import {
  ...
  Trash2, Send, Globe, Clock, DollarSign,
} from 'lucide-react'
import PreciosTab from '@/components/eventos/PreciosTab'
...
export default function EventoDetalleClient({ eventoId }: { eventoId: string }) {
  ...
  const [tabActual, setTabActual] = useState<'descripcion' | 'precios' | 'prensa'>('descripcion')
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session: s } }) => {
      setSession(s)
      if (!s) setIsGuest(localStorage.getItem('isGuest') === 'true')
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">
                Mi Interés en Asistir
              </label>
              <div className="space-y-3">
                <select
                  disabled={loading || isGuest}
                  className={`w-full text-sm border border-slate-200 rounded-xl focus:ring-blue-500 focus:border-blue-500 py-3 px-4 font-semibold text-slate-700 bg-slate-50 transition-all ${isGuest ? 'opacity-60 cursor-not-allowed' : ''}`}
                ...
                {estadoAsistencia && !isGuest && (
                  <button
                    ...
                    disabled={deleting}
                    className="w-full flex items-center justify-center gap-2 py-2 text-xs font-bold text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
                  >
          ...
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl overflow-hidden border border-white/20">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-1">
                <label className="block text-xs font-medium text-slate-600">Para (destinatario)</label>
                <input
                  type="email"
                  value={shareEmail}
